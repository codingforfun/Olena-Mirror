...
namespace mln
{
  ...
  namespace utils
  {
    ...
    template <typename Q>
    void
    push_frontiere_at_q_level(Q& q, image2d<bool>& deja_vu)
    {
      const int
	nrows = deja_vu.nrows(),
	nrows_1 = nrows - 1,
	...
	ncols_1 = ncols - 1;
      unsigned p,
	dp = deja_vu.delta_index(dpoint2d(1, 0));

      // top
      p = deja_vu.index_of_point(point2d(0, 0));
      for (int col = 0; col < ncols; ++col, ++p)
	{
      ...
      // bot
      p = deja_vu.index_of_point(point2d(nrows_1, 0));
      for (int col = 0; col < ncols; ++col, ++p)
	{
	  q.push_element_at_current_level(p);
	  deja_vu.element(p) = true;
      ...
      // left
      p = deja_vu.index_of_point(point2d(1, 0));
      for (int row = 1; row < nrows_1; ++row, p += dp)
	{
	  q.push_element_at_current_level(p);
	  deja_vu.element(p) = true;
      ...
      // right
      p = deja_vu.index_of_point(point2d(1, ncols_1));
      for (int row = 1; row < nrows_1; ++row, p += dp)
	{
	  q.push_element_at_current_level(p);
	  deja_vu.element(p) = true;
