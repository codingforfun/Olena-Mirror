...
\code{MIN} and \code{MAX}.  \code{BEHAVIOR} specify how overflow
...
\code{range<int_u<8>, 10, 25>} is an unsigned integer,
stored on 8 bits, but restricted to the $[10...25]$ interval.  Any
attempt to escape this interval will abort the program.  (Because
\code{strict} is the default behavior.)

% \code{range} changes only the bounds of a type, not
% the behavior.  So \code{range<int_u16c, 0, 359>}, a integer cycling
% over the range $[0...359]$, can be useful to represent an angle in
% degrees.

\code{MIN} and \code{MAX} are always integers (the C++ language does not
allow floats as template arguments), so to define a \code{sfloat}
between $0$ and $1$, use \code{range<sfloat,0,1>}.

%FIXME: How would we define a float \emph{cycling} over $[0...360[$?

\subsubsection{\lstinline$cycle<T, MIN, MAX>$}

This class defines a type, the values of which can be built from
\code{T}, and that cycle between \code{MIN} (included) and
\code{MAX} (excluded).
...
%% \subsubsection{\lstinline$view_as<T,U>$}
%%
%% \lstinline$view_as<T,U>$ declares a variable stored in type \code{T}
...
\lstinline$bin$ is a binary type, with two values: \code{0}
and \code{1}.  This is unlike the built-in \lstinline$bool$, whose
...
\lstinline$label<T, BEHAVIOR=cycle>$ is used to label images.
...
For a undecorated type \code{T}, \lstinline$typetraits<T>::base_type$ is
\code{T} itself.
...
%% For example,
%% \lstinline$typetraits< view_as<int_u8, range<sfloat, 0, 1> > >::view$
%% is \lstinline$range<sfloat, 0, 1>$.
%%
%% For a undecorated type \code{T}, \lstinline$typetraits<T>::view$ is
%% \code{T} itself.
%%
%% Note that the view of \code{fuzzy16} as defined above, i.e,
%% \lstinline$typetraits< view_as<int_u8, view_as<int_u16, range<sfloat, 0, 1> > > >::view$
%% is still \lstinline$range<sfloat, 0, 1>$.

\item[\code{signed_type}]
The smallest signed type that can countain the values of \code{T}, if possible.
For instance
\lstinline$typetraits< int_u8 >::signed_type$ is \code{int_s<10>}.
(\code{int_s<9>} is not large enough to hold the value 256.)

However \lstinline$typetraits< int_u32 >::signed_type$ is \code{int_s<32>}.

\item[\code{unsigned_type}]
The smalled unsigned type that can countain the non negative values of
\code{T}.
For instance,
\lstinline$typetraits< int_s<9> >::unsigned_type$ is \code{int_u8}.
...
For instance
\lstinline$typetraits< int_u8 >::cumul_type$ is \code{int_u16}.

\item[\code{largest_type}]
The largest type in the familly of \code{T}.  For instance
\lstinline$typetraits< int_u8 >::largest_type$ is \code{int_u32}.
...
For instance \lstinline$typetraits< int_u8 >::interger_type$ is \code{unsigned
int}; \lstinline$typetraits< sfloat >::interger_type$ is \code{signed
int}.
