...
\subsubsection{\code{range<T, MIN, MAX, BEHAVIOR=strict>}}
...
\subsubsection{\code{cycle<T, MIN, MAX>}}

This class defines a type, the values of which can be built from
\code{T}, and that cycle between \code{MIN} (included) and
\code{MAX} (excluded).

For instance \code{cycle<sfloat, 0, 360>} defines a type which is
useful to represent angles in degrees.

Any value implicitely convertible to \code{T} can be involved in an
arithmetic operation with \code{cycle<T, MIN, MAX>}.

Any conversion from \code{cycle<T, A, B>} to
\code{cycle<T, C, D>} must be explicit and should
...
This type is used by the \code{range<T, MIN, MAX, B>} template
to define the type of it's arguments: \code{MIN} and \code{MAX} both
have type \code{typetraits< T >::interger_type}.
