...
For brevity, the \code{range_u} and \code{range_s} shorthands are
supported.  The above example could be rewritten as
\code{range_u<int_u<8>, 10, 25 >}
...
as in \code{range}.  Similarly, the \code{cycle_u} and \code{cycle_s}
shorthands are supported.

For instance \code{typedef cycle_u<sfloat, 0, 360> deg} defines a type
which is useful to represent angles in degrees.  \code{deg d = 400.0}
will set \code{d} to \code{40.0}.
