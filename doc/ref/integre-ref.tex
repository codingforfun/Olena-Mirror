...
This section describes the basic types available with \integre. By
\emph{basic types} we roughly mean any type that can be used to store
a value, such as \code{int}, \code{float}, etc.  This also includes
composite types like complexes, vectors, and colors.
...
Here is a list of basic types supported by \integre.  Although it is
  ...
  \item Integers
    ...
    \item Signed
      ...
      \item C built-in types:
        ...
        \end{list}
      \item \integre type: \code{int_s<BITS, BEHAVIOR>}
      \end{list}
    \item Unsigned
      \begin{list}{--}{}
      \item C built-in types
        ...
        \end{list}
      \item \integre types: \code{int_u<BITS, BEHAVIOR>}
      \end{list}
    \end{list}
  \item Decimal
    \begin{list}{--}{}
...
\item Enumerated types
  \begin{list}{--}{}
  \item C built-in type: \code{bool}
  \item \integre type: \code{bin}
  \end{list}
\item Vectorial types
  \begin{list}{--}{}
  \item \integre type: \code{cplx<R, T>}
    \begin{list}{--}{}
    \item \code{cplx<rect, T>}: complex with $a+ib$ representation
    \item \code{cplx<polar, T>}: complex with $r\exp{i\theta}$ representation
    \end{list}
  \item \code{vec<N, T>}
  \item \code{colors}
  \end{list}
...
\subsection{\code{int_u<N, BEHAVIOR=strict>}}
...
This behavior is useful for tasks like histogram construction where a
...
\subsection{\code{float_s} and \code{float_d}}

These are the Olena equivalents of \code{float} and \code{double},
Contrary to signed and unsigned integer types, there is not
\emph{extension} during operations.  Multiplying two \code{float_s}
yields a \code{float_s}.
...
not allow floats as template arguments), so to define a \code{float_s}
between $0$ and $1$, use \code{range<float_s,bounded_u<0,1> >}, not
\code{range<float_s,bounded_u<0.0,1.0> >}. }%%%
...
For instance \code{typedef cycle<float_s, bounded_u<0, 360> > deg}
...
Any value implicitly convertible to \code{T} can be involved in an
...
%% \code{float_s} values between 0 and 1.
%% \begin{lstlisting}{}
%% typedef view_as<int_u8, range<float_s, 0, 1> > fuzzy8;
%% \end{lstlisting}
%%
...
%% which are seen like \code{fuzzy8}, i.e., a \code{float_s} value between
...
%% For example,
%% \code{typetraits< view_as<int_u8, range<float_s, 0, 1> > >::view}
%% is \code{range<float_s, 0, 1>}.
%%
%% For a undecorated type \code{T}, \code{typetraits<T>::view} is
%% \code{T} itself.
%%
%% Note that the view of \code{fuzzy16} as defined above, i.e,
%% \code{typetraits< view_as<int_u8, view_as<int_u16, range<float_s, 0, 1> > > >::view}
%% is still \code{range<float_s, 0, 1>}.

\item[\code{signed_type}]
The smallest signed type that can contain the values of \code{T}, if possible.
For instance
...
int}; \code{typetraits< float_s >::interger_type} is \code{signed
int}.
