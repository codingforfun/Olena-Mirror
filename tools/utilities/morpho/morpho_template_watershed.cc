...
{
  std::cout << "'" << progname 
	    << "' calls Olena function morpho::" MAKE_STRING(FUNC<DATA_TYPE>) 
    "(" MAKE_STRING(IMAGE_TYPE<DATA_TYPE>) ", " MAKE_STRING(NEIGHBORHOOD_TYPE) ")" << std::endl
	    << std::endl
	    << "Usage: " << progname << " [-n NEIGHFILE] INFILE OUTFILE" << std::endl
	    << std::endl
	    << " -n filename, --neighb filename     load a neighborhood from file [neighb_c4]" << std::endl
	    << std::endl
	    << "Example: " << progname << " -n my_neighb.pbm lena.pgm lena_out.pgm" << std::endl
	    << std::endl
