...
namespace oln {
  ...
  namespace io {
	...
	static bool read(std::istream& in, image2d<bin>& im)
	{
      ...
      struct reader<ReadPnmPlain, image2d<int_u<N, behavior> > >
      {
	static const std::string& name()
	...
	static bool read(std::istream& in, image2d<int_u<N, behavior> >& im)
	{
	...
	static bool read(std::istream& in, image2d<bin>& im)
	{
      ...
      template <unsigned N, class behavior>
      struct reader<ReadPnmRaw, image2d<int_u<N, behavior> > >
      {
	static const std::string& name()
