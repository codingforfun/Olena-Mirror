...
#               [CONFIG-FILES],
#               [EXTRA-ACTION-IF-SELECTED],
#		[DIRNAME-FOR-SUBDIRS-IF-DIFFERENT-THAN-FIRST-DIRNAME])
...
AC_DEFUN([OLN_NON_COMPONENT], [dnl
AC_CACHE_CHECK([whether to build $4],
               [$3],
               [$3=no
                if test -r "$srcdir/$1/Makefile.in"; then
                   AC_ARG_WITH([$2],
                               [AC_HELP_STRING([--with-$2], [enable building $4])],
                               [$3=$withval],
                               [$3=no])
                fi])
if test x$[]$3 != xno; then
