...
namespace mlc
{
  ...
  struct binary_traits <Tag, array3d<Info_, T_>, U>
  {
    typedef typename binary_traits<Tag, T_, U>::ret V;
    typedef array3d<Info_, V> ret;
  };
  ...
  struct array3d
  {
    ...
    name()
    {
      return std::string("array3d< Info, ") + mlc_to_string(T) + " >";
    }
    ...
    template <class U>
    array3d< Info, mlc_binary_ret(times, T, U) >
    operator*(U w)
    {
      array3d< Info, mlc_binary_ret(times, T, U) > tmp;
    ...
    template <class U>
    array3d< Info, mlc_binary_ret(div, T, U) >
    operator/(U w)
    {
      array3d< Info, mlc_binary_ret(div, T, U) > tmp;
      for (unsigned i = 0; i < Info::card; ++i)
	tmp[i] = this->buffer_[i] / w;
      return tmp;
    }
